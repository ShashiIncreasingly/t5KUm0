(function(){
    ...
    var cssstyle = ".inc_goes_img_block{position:relative}.inc_promo_qty_block{display:flex;position:absolute;top:auto;bottom:0;right:0;background:#F4C838;color:#000;font-weight:700;height:70px;width:70px;text-align:center;justify-content:center;align-items:center;border-radius:40px}span.inc_promo_qty_value{font-size:40px;font-weight:700}.inc_promo_qty_text{font-size:24px;font-weight:700;margin-bottom:-7px}@media only screen and (max-width: 768px) {.inc_promo_qty_block{height:50px;width:50px;}span.inc_promo_qty_value{font-size:28px;}.inc_promo_qty_text{font-size:18px;}}";
    ...
    INC.clientConfig.offset_fbtRecs = 0;
    INC.clientConfig.slidevalue_deal = 0;
    INC.clientConfig.slidevalue_fbtRecs = 0;
    INC.config = {};
    INC.config.qtyDrill = [];
    ...
    INC.config.FBTTitle = "よく一緒に購入されている商品"
    INC.config.FBTTitleJp = "よく一緒に購入されている商品"
    ...
    INC.config.dealprodplaced = false;
    var fabricprod=[];
    ...
    INC.init = function () {
        ...
        switch (INC.config.pageType) {
            case "pdp":
                ...
                if (document.querySelector('.goods-body .stock-status').textContent == '完売' || document.querySelector('.goods-body .stock-status').textContent == '在庫なし' || document.querySelector('.goods-body .stock-status').textContent == 'お取寄せ（通常納期：1～2週間程度）'){
                    oosStatus = true;
                } else {
                    oosStatus = false;
                }
            ...
            case "productList":
                ...
                if(elbrdscrub[0] != undefined){
                    ...
                    INC.config.plpCatText = elcatids.trim();               }
                ...
                if (Object.prototype.hasOwnProperty.call(bundles, i)) {
                    if (INC.config.dupDataObjectProducts.indexOf(bundles[i].BundleId) == -1 || window.location.pathname == "/basket" || window.location.pathname == "/basket/") {
                        ...
                        if (INC.config.recommendation == falseflag) {
                            ...
                            if(irbBundleObj["BundleLevelDiscountMessage"] != null){
                                if (INC.config.qtyDrill.length == 0){
                                    INC.config.qtyDrill.push(irbBundleObj.BundleId);
                                }
                            }
                        }
                                                ...
                                                if (Object.prototype.hasOwnProperty.call(pAttributes, pattv)) {
                                                    ...
                                                    for (var ptr in pAttributesValues) {
                                                        if (Object.prototype.hasOwnProperty.call(pAttributesValues, ptr)) {
                                                            ...
                                                            if(productObj.bundleId == undefined){
                                                                productObj.bundleId = bundles[i].BundleId
                                                            }
    ...
    INC.classes.ProductObj = function (product) {
        ...
        this.skinType = ((product != null) && (product != undefined)) ? product["DealName"] : "";
        ...
        this.dealtype =  ((product != null) && (product != undefined)) ? product["DealType"] : "";
        this.ProductType = ""
    ...
    INC.methods.irbReq = function (productId, bundleCount, elementName) {
        ...
        if (elementName == "checkout_cart_index"){
            ...
            irbReqParams = "product_ids=" + productIdReq +  "&category_id=" +  "&api_key=" + apiKey +  "&page_type=" + elmname  +  "&fd=" + "1" +  "&fr=" + "1" + "&client_visitor_id=" + INC.config.ivid + "&no_of_bundles=" + 10
        }
                ...
                switch (INC.config.pageType) {
                    case "pdp":
                        if ((Object.prototype.hasOwnProperty.call(responseJSON, "Bundles") && (responseJSON.Bundles != null))) {
                            ...
                            if (responseJSON.ProductsDetail.length > 3){
                                var fbtRecs = INC.dataStore.bundleProductsArray.splice(0,1)
                                INC.config.FbtRecs = INC.dataStore.bundleProductsArray;
                                INC.methods.createAMBlock("PDP",'fbtRecs')
                            }
                        }else{
                            setTimeout(function(){
                                if(INC.config.DealProducts.length > 0){
                                    if(INC.config.dealprodplaced == false){
                                        if (oosStatus == false){
    ...
    INC.methods.irbReq_is_tc = function (productIds, bundleCount, elementNameel,elcatids) {
        ...
        var apiKey = INC.config.clientToken;
        var irbPATH = "//jprapidload.increasingly.co/increasingly_bundles?irb/";
        var irbReqParams = "product_ids=" + productId + "&category_id=" + "&api_key=" + apiKey + "&rc=1&client_id=" + "&page_type=" + elementNameel + "&fr=" + "1" + "&client_visitor_id=" + INC.config.ivid +""
    ...
    INC.methods.irbReq_is_categ_recomendation = function (productId, _bundleCount, elementName,categoryid) {
        ...
        if(categoryid != null){
            ...
            irbReqParams = "product_ids=&category_id="+catid + "&api_key=" + apiKey + "&client_id=" + "&page_type=" + elementName_ + "&fr=" + "1" + "&client_visitor_id=" + INC.config.ivid + ""
            if (INC.config.pageType == "pdp") {
            ...
            irbReqParams = "product_ids=" + productId_ + "&category_id=" + "&api_key=" + apiKey + "&rc=1&client_id=" + "&page_type=" + elementName_ + "&fr=" + "1" + "&client_visitor_id=" + INC.config.ivid +""
        }
    ...
    INC.methods.createAMBlock = function (PDP_type,categorytype) {
        if (INC.dataStore.dataStoreObj.undefined == undefined) {
            ...
            if (PDP_type == "PDP") {
                ...
                }else if(categorytype == "fbtRecs"){
                    amBlock.classList.add('inc_fbtRecs_blk')
                    elAmTitleText.classList.add('inc_fbtRecs_blk')
                }else{
            ...
            if (PDP_type == "PDP") {
                ...
                if(categorytype == "deal"){
                    ...
                    if (INC.config.DealProducts.length >= 3){
                        bundleProductsArray = INC.config.DealProducts;
                        recstype='deal';
                    }
                }
                if(categorytype == "fbtRecs"){
                    console.log('RECSTYPE: FbtRecs')
                    if (INC.config.FbtRecs.length >= 3){
                        bundleProductsArray = INC.config.FbtRecs;
                        recstype='fbtRecs';
                    }
                }
                ...
                bundleProductsArray.forEach(function (mainProductId) {
                    if(duplamid.indexOf(mainProductId) == -1){
                        ...
                        if(categorytype == "fbtRecs"){
                            if (typeof(firstProductObjByMainId.bundleId) != 'string'){
                        ...
                        if(categorytype == "categorytype"){
                            ...
                            let deal_Name = decodeURIComponent(firstProductObjByMainId.skinType)
                            let deal_hiperLink = "";
                            if (firstProductObjByMainId.skinType != ""){
                                deal_Name = decodeURIComponent(firstProductObjByMainId.skinType).split('|')[0]
                                if(decodeURIComponent(firstProductObjByMainId.skinType).split('|')[1]){
                                    deal_hiperLink = decodeURIComponent(firstProductObjByMainId.skinType).split('|')[1]
                                }
                            }
                            if (document.querySelector('.inc_pdp_block') != null){
                                if (document.querySelector('.inc_pdp_block').style.display == 'none'){
                                    ...
                                    if (firstProductObjByMainId.dealtype == 2){
                                        if (firstProductObjByMainId.skinType != ""){
                                            dealType = firstProductObjByMainId.dealtype;
                                            amBlock.classList.add('deal_p_2')
                                            elAmTitleText.innerText = deal_Name
                                            if(deal_hiperLink != "" && deal_hiperLink != undefined){
                                                if (deal_Name == 'おすすめ関連商品 対象マザーボードと同時購入で2,000ポイントプレゼント！詳細はこちら(7/28まで) '){
                                                    elAmTitleText.innerText = "おすすめ関連商品"
                                                    elAmTitleText1.style.fontSize="14px";
                                                    elAmTitleText1.style.fontWeight="bold";
                                                    elAmTitleText1.innerHTML = '対象マザーボードと同時購入で'
                                                    +'<span class="redColor" style=color:#e00d0d !important>2,000ポイント</span>'+'プレゼント！詳細は'
                                                    +'<a href='+deal_hiperLink+' class="greenColor" style="text-decoration: underline;">こちら</a>'
                                                    +'(7/28まで)'
                                                } else {
                                                    elAmTitleText.innerHTML = '<a href='+deal_hiperLink+' style="text-decoration: none;">'+deal_Name+'</a>'
                                                }
                                            }
                                        }
                                    } else {
                            ...
                            elAmTitleText.innerText = "この商品を買った人は、こんな商品も買っています"
                        } else {    
                            var elbrdscrub= document.querySelectorAll('.topic-path-block .topic-path li')
                            var elcatlast="";
                            var elcatbutone = "";
                                ...
                                if (prd_module != undefined){
                                    if (prd_module.getAttribute('data-id') != "undefined") {
                                        ...
                                        if (INC.config.pageType == 'productList'){
                                            ...
                                            if(lastMonday.toString().indexOf('Mon') == -1){
                                                lastMonday = new Date();
                                            ...
                                            var month = lastMonday.getMonth() + 1;
                                            if (month < 10){
                                                month = "0"+month
                                            } 
                                            var year = lastMonday.getFullYear()
                                            var dateString1 = parseInt(lastMonday.toDateString().split(' ')[2]);
                                            var dateString = dateString1.toString()
                                            if(dateString.toString().length < 2){
                                                dateString = "0" + dateString
                                            }
                                            if(lastMonday.toString().indexOf('Mon') >= 0){
                                                elAmTitleText.innerHTML = INC.config.plpCatText+INC.config.PlpText + "<span>"+" 更新日: " +year+"/"+month+"/"+dateString+ "</span>";
                                            }else{
                                                elAmTitleText.innerHTML = INC.config.plpCatText+INC.config.PlpText
                                            }
                                        } else {
            ...
            if (PDP_type == "PDP") {
                if (categorytype == "deal" || categorytype == "fbtRecs"){
                    if (categorytype == "deal"){
                    ...
                    if (document.querySelector('.inc_pdp_block') != null){
                        ...
                        if (categorytype == "fbtRecs"){
                            if (document.querySelector('.inc_af_block.deal_p') != null){
                                var fbtRecs = document.querySelector('.inc_af_block.deal_p');
                                pdplc = fbtRecs.nextElementSibling
                            } else {
                                pdplc = pdpplacement.nextElementSibling;
                            }
                        ...
                        if (document.querySelector('.goods-body section .size_select_area .itembox') != null || document.querySelector('.goods-body section .size_select_area') != null){
                            if (document.querySelectorAll('.size_select_area').length == 2 || document.querySelectorAll('.size_select_area').length == 1){
                                if (document.querySelectorAll('.size_select_area').length == 1 || dealType == 2) {
                                    if (document.querySelector('.main-contents__recommend-frame') != null || dealType == 2){
                                        if (dealType == 2){
                                            pdpBlockSibling = document.querySelector('#spec-contents')
                                        } else if (document.querySelector('.main-contents__recommend-frame.mb20__common h2').innerText == '関連オススメ商品' && window.location.pathname == '/goods/0763649161753/'){
                                        ...
                                        if (window.location.pathname == '/goods/4549576188379/' || window.location.pathname == '/goods/4549576188386/' || window.location.pathname == '/goods/4549576188393/'){
                                            ...
                                            if(document.querySelector('.inc_af_block.af_cart_page.inc_normal_blk') != null){
                                                ...
                                                if (document.querySelector('.mb20__common .swiper-container') != null){
                                                    if (document.querySelector('.o_container') != null){
                                                        pdpBlockSibling = document.querySelector('.o_container').previousElementSibling;
                                                    } else {    
                                                        if (document.querySelector('#slider') != null ){
                                                            pdpBlockSibling = document.querySelector('#slider').previousElementSibling;
                                                        }
                                                    }
                                                } else if (document.querySelector('.mb20__common .p20 .swipebox') != null){
                                                    pdpBlockSibling = document.querySelector('.mb20__common .tile01').parentNode.querySelector('.title01__common');
                                                } else {
                                                    if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common') != null){
                                                        if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common').innerText == '製品画像' || document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common').innerText == '' || document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common').innerText == 'デバイスからPCパーツまで幅広く手掛けるADATA/XPGブランド製品をご紹介！ ADATA製品特集'){
                                                            pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common')
                                                        }
                                                    } else if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.size_select_area') != null){
                                                        pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.size_select_area').nextSibling;
                                                    } else if(document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('img') != null){
                                                        pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling;
                                                    } else {
                                                        pdpBlockSibling = document.querySelector('.main-contents__spec-frame');
                                                    }
                                                }
                                ...
                                if (document.querySelectorAll('.size_select_area').length >= 2) {
                                    ...
                                    if (a != null){
                                        ...
                                        if(a.nextElementSibling != null){
                                            ...
                                            pdpBlockSibling = a.parentNode.nextElementSibling;
                                        }
                                    } else {
                            ...
                            if (document.querySelector('.swiper-containerr') != null){
                                ...
                                if (document.querySelector('.main-contents__recommend-frame') != null){
                                    if (document.querySelector('.mb20__common .main-contents__intel').innerText.indexOf('ツクモ延長保証について') > 0){
                                        if (document.querySelector('.mb20__common .main-contents__intel').parentElement != null){
                                            if (document.querySelector('.mb20__common .main-contents__intel').parentElement.parentElement.querySelector('.stcmt_ttl').innerText == '担当スタッフのひとことコメント'){
                                                pdpBlockSibling = document.querySelector('.mb20__common .main-contents__intel').parentElement.parentElement.querySelector('.stcmt_ttl');
                                            }
                                        } else {
                            ...
                            if (document.querySelector('.goods-body section .size_select_area .itembox') != null || document.querySelector('.goods-body section .size_select_area') != null){
                                if (document.querySelectorAll('.size_select_area').length == 2 || document.querySelectorAll('.size_select_area').length == 1){
                                    if (document.querySelectorAll('.size_select_area').length == 1) {
                                        ...
                                        if (document.querySelector('.mb20__common .o_container') != null){
                                            pdpplacement = document.querySelector('.mb20__common .o_container').previousElementSibling
                                        } else if (document.querySelector('#spec-contents') != null){
                                    ...
                                    if (document.querySelectorAll('.size_select_area').length >= 2) {
                                        ...
                                        if (document.querySelector('#spec-contents') != null){
                                            pdpplacement = document.querySelector('#spec-contents');
                                        } else if (a.nextElementSibling != null){
                                            pdpplacement = a.nextElementSibling;
                                        } else {
                                            if (document.querySelector('.main-contents__recommend-frame') != null){
                                                pdpplacement = document.querySelector('.main-contents__recommend-frame');
                                ...
                                if (document.querySelector('.mb20__common .stcmt_ttl')){
                                    pdpplacement = document.querySelector('.mb20__common')
                                } else if (document.querySelector('#spec-contents') != null){
                                    pdpplacement = document.querySelector('#spec-contents')
                        ...
                        if (document.querySelector('.main-contents__recommend-frame.mb20__common h2') != null) {
                            ...
                            if (categorytype == 'categorytype'){
                                if(document.querySelector('.main-contents__recommend-frame.mb20__common h2') != null){
                                    ...
                                    if (document.querySelector('.mb20__common #slider') != null){
                                        if (document.querySelector('.inc_pdp_block') != null){
                                            if (document.querySelector(".inc_af_block.deal_p") != null){
                                                var pdp = document.querySelector(".inc_af_block.deal_p");
                                                let pdplc = pdp.nextElementSibling 
                                                pdpBlockSibling = pdplc;
                                                pdpplacement = pdplc;
                                            } else {
                                                var pdp = document.querySelector('.inc_pdp_block');
                                                let pdplc = pdp.nextElementSibling 
                                                pdpBlockSibling = pdplc;
                                                pdpplacement = pdplc;
                                            }
                                        } else {
                                            pdpBlockSibling = document.querySelector('.mb20__common #slider')
                                            pdpplacement = document.querySelector('.mb20__common #slider')
                                        }
                                    } else {
                                        if (document.querySelector('.main-contents__spec-frame') != null){
                                            if (document.querySelector('.inc_pdp_block') != null){
                                                if (document.querySelector(".inc_af_block.inc_fbtRecs_blk") != null){
                                                    let pdplc;
                                                    if (document.querySelector('#spec-contents') != null){
                                                        var pdp = document.querySelector('#spec-contents');
                                                        pdplc = pdp
                                                    } else if (document.querySelector('.mb20__common .main-contents__intel') != null){
                                                        var pdp = document.querySelector('.mb20__common .main-contents__intel');
                                                        let pdplc = pdp
                                                        pdpBlockSibling = pdplc;
                                                        pdpplacement = pdplc;
                                                    } else {
                                                        var pdp = document.querySelector('.main-contents__spec-frame');
                                                        let pdplc = pdp.nextElementSibling 
                                                        pdpBlockSibling = pdplc;
                                                        pdpplacement = pdplc;
                                                    }
                                ...
                                if (document.querySelector('.goods-body .main-contents__spec-frame') != null){
                                    ...
                                    if (document.querySelector('.inc_pdp_block') != null){
                                        if (document.querySelector(".inc_af_block.inc_fbtRecs_blk") != null){
                                            let pdplc;
                                            if (document.querySelector('.mb20__common #slider') != null){
                                                var pdp = document.querySelector('.mb20__common #slider').parentNode
                                                pdplc = pdp
                                            } else if (document.querySelector('#spec-contents') != null){
                                                var pdp = document.querySelector('#spec-contents');
                                                pdplc = pdp
                                                ...
                                                var pdp = document.querySelector('.mb20__common .main-contents__intel');
                                                pdplc = pdp
                                            } else if (document.querySelector('.mb20__common #slider') != null){
                                                var pdp = document.querySelector(".inc_af_block.inc_fbtRecs_blk")
                                                pdplc = pdp.nextElementSibling 
                                            } else {
                                                var pdp = document.querySelector('.main-contents__spec-frame');
                                                pdplc = pdp.nextElementSibling 
                                        ...
                                        if (document.querySelector('#spec-contents') != null){
                                            pdpBlockSibling = document.querySelector('#spec-contents')
                                            pdpplacement = document.querySelector('#spec-contents')
                                        } else if(document.querySelector('#review') != null){
                                            pdpBlockSibling = document.querySelector('#review')
                                            pdpplacement =document.querySelector('#review')
                                        }
                                    }
            ...
            setTimeout(function(){
                if (document.querySelector('.inc_af_block.inc_fbtRecs_blk') != null){
                    if (document.querySelector('.inc_af_block.inc_fbtRecs_blk .inc_af_product-list-main_block .inc_product_module_block') != null){
                        var len = document.querySelectorAll('.inc_af_block.inc_fbtRecs_blk .inc_af_product-list-main_block .inc_product_module_block').length;
                        if (len <= 6){
                            document.querySelector('.inc_af_block.inc_fbtRecs_blk .inc_af_bundles_block').classList.add('less_product')
                        }
                    }
                }
            },300)
            ...
            var sliderBtnCounter_fbtRecs = 0;
            ...
            amBlock.querySelector('.inc_af_right_btn_block').addEventListener("click", function (e) {
                ...
                if(categorytype == "categorytype"){
                    ...
                    sliderBtnCounter_fbtRecs++;
                    if (sliderBtnCounter_fbtRecs > productBlocks.length - 1) {
                        sliderBtnCounter_fbtRecs--;
                    }
                } else{
                    sliderBtnCounter++;
                ...
                if(categorytype == "categorytype"){
                    ...
                    if (+INC.clientConfig.slidevalue_fbtRecs + +cc_ounts < all_p_length) {
                        elblock.querySelector('.inc_af_left_btn_block').setAttribute("style", "pointer-events: auto;opacity:1");
                        elblock.querySelector('.inc_af_left_btn_img').setAttribute("style", "pointer-events: auto;opacity:1");
                         INC.clientConfig.slidevalue_fbtRecs++
                         INC.clientConfig.offset_fbtRecs = (parseInt(INC.clientConfig.offset_fbtRecs) - width_slide) + 'px';
                         var menu =elblock.querySelector('.inc_af_product-list_block');
                         menu.setAttribute('style', 'margin-left:' + INC.clientConfig.offset_fbtRecs + ';transition-duration:0.5s;-webkit-transition-duration:0.5s;-moz-transition-duration:0.5s;-ms-transition-duration:0.5s;-o-transition-duration:0.5s;')
                         if (document.querySelector('.inc_af_product-list-dots-main_block .inc_dots.inc_active') != null) {
                             document.querySelector('.inc_af_product-list-dots-main_block .inc_dots.inc_active').classList.remove('inc_active')
                         }
                         if (document.querySelectorAll('.inc_af_product-list-dots-main_block .inc_dots')[+INC.clientConfig.slidevalue_fbtRecs] != null) {
                             document.querySelectorAll('.inc_af_product-list-dots-main_block .inc_dots')[+INC.clientConfig.slidevalue_fbtRecs].classList.add('inc_active')
                         }
                     }
            ...
            amBlock.querySelector('.inc_af_left_btn_block').addEventListener("click", function (e) {
                ...
                if(categorytype == "categorytype"){
                    ...
                    if (INC.clientConfig.slidevalue_fbtRecs == -1){
                        INC.clientConfig.slidevalue_fbtRecs = 0;
                    }
                } else{
                ...
                if(categorytype == "categorytype"){
                    ...
                    if (INC.clientConfig.slidevalue_fbtRecs > 0) {
                        elblock_.querySelector('.inc_af_right_btn_block').setAttribute("style", "pointer-events: auto;opacity:1");
                        elblock_.querySelector('.inc_af_right_btn_img').setAttribute("style", "pointer-events: auto;opacity:1");
                        INC.clientConfig.slidevalue_fbtRecs--
                        INC.clientConfig.offset_fbtRecs = (parseInt(INC.clientConfig.offset_fbtRecs) + width_slide) + 'px';
                        ...
                        if (elblock_.querySelectorAll('.inc_af_product-list-dots-main_block .inc_dots')[+INC.clientConfig.slidevalue_fbtRecs] != null) {
                            elblock_.querySelectorAll('.inc_af_product-list-dots-main_block .inc_dots')[+INC.clientConfig.slidevalue_fbtRecs].classList.add('inc_active')
                        }
                    }
                    if (INC.clientConfig.slidevalue_fbtRecs == 0) {
                        elblock_.querySelector('.inc_af_left_btn_block').setAttribute("style", "pointer-events: none;opacity:0.5");
        ...
        if (sidebar != "sidebar") {
            if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == falseflag) {
                ...
                for (var i = 0; i < productVisibleCount; i++) {
                    ...
                    if (firstProductObjByMainId.bundleId != 'offer0' && firstProductObjByMainId.bundle_promotion != null && INC.config.qtyDrill.indexOf(firstProductObjByMainId.bundleId) > -1){
                        ...
                        if (firstProductObjByMainId.bundleId != 'offer0' && (INC.config.qtyDrill == undefined || INC.config.qtyDrill == null || INC.config.qtyDrill.length == 0)){
                            var productBlock = this.createProductModuleBlock(firstProductObjByMainId.id, "pdp", true, pdpBlock);
                            productBlock.setAttribute('b-index', i + 1);
                            pdpBundleProductListMainBlock.appendChild(productBlock);
                            var productListItemBlock = this.createProductListItemBlock(firstProductObjByMainId, "pdp");
                ...
                if(pdpBlock.querySelector('.inc_pdp_bundle_product_list_main_block .inc_product_img_main_img img') != null){
                        ...
                        if (document.querySelectorAll('.size_select_area').length == 2 || document.querySelectorAll('.size_select_area').length == 1){
                            if (document.querySelectorAll('.size_select_area').length == 1 || document.querySelectorAll('.size_select_area').length == 2) {
                                if (document.querySelector('.main-contents__recommend-frame') != null){
                                    ...
                                    if(document.querySelector('.mb20__common .stcmt_ttl') != null){
                                        if (document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container') != null){
                                            if (document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container').previousElementSibling != null){
                                                if (document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container').previousElementSibling.innerText == '製品画像'){
                                                    pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container').previousElementSibling
                                                }
                                            } else if (document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.stcmt_ttl').innerText == '担当スタッフのひとことコメント'){
                                                pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.stcmt_ttl')
                                            } else if (document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container').parentNode.className == 'o_container'){
                                                pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl').parentElement.querySelector('.swiper-container').parentNode.previousElementSibling
                                            } else {
                                                pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl')
                                            }
                                        } else {
                                            if (document.querySelector('.mb20__common .stcmt_ttl').parentNode.previousElementSibling != null){
                                                if (document.querySelector('.mb20__common .stcmt_ttl').parentNode.previousElementSibling.querySelector('.swiper-container') != null){
                                                    pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl').parentNode.previousElementSibling.querySelector('.o_container').previousElementSibling
                                                } else {
                                                    pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl').parentNode.previousElementSibling
                                                }
                                            } else {
                                                pdpBlockSibling = document.querySelector('.mb20__common .stcmt_ttl');
                                            }
                                        ...
                                        if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.o_container') != null){
                                            if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.o_container').previousElementSibling.innerText == "製品画像"){
                                                pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.o_container').previousElementSibling
                                            }
                                        } else if (document.querySelector('.mb20__common .p20 .swipebox') != null){
                                            pdpBlockSibling = document.querySelector('.mb20__common .tile01').parentNode.querySelector('.title01__common');
                                        } else if (document.querySelector('.mb20__common .swiper-container') != null){
                                            ...
                                            if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common') != null){
                                                if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common').innerText == 'デバイスからPCパーツまで幅広く手掛けるADATA/XPGブランド製品をご紹介！ ADATA製品特集' || document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('.title01__common').innerText == 'Noctua ファンのススメ' || window.location.href == 'https://nile-prev2017.tsukumo.co.jp/goods/4582535432502/'){
                                                    pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling
                                                } else {
                                                    if (document.querySelector('#spec-contents') != null){
                                                        pdpBlockSibling = document.querySelector('#spec-contents');
                            ...
                            if (document.querySelectorAll('.size_select_area').length >= 2) {
                                ...
                                if (a.nextElementSibling != null){
                                    ...
                                    if (document.querySelector('.main-contents__recommend-frame') != null){
                                        ...
                                        if (document.querySelector('.main-contents__spec-frame').previousElementSibling.querySelector('img') != null){
                                            pdpBlockSibling = document.querySelector('.main-contents__spec-frame').previousElementSibling
                                        } else {
                                            pdpBlockSibling = document.querySelector('.main-contents__spec-frame');
                                        }
            ...
            if(INC.config.DealProducts.length > 0){
                INC.config.dealprodplaced = true
                INC.methods.createAMBlock("PDP",'deal')
            }
        }
        if (sidebar == "sidebar") {
    ...
    INC.methods.sendBundleClickTracking = function(productId, categoryRecs,recstype,bundleid) {
        var pageType = "";
        switch (INC.config.pageType) {
            case "pdp":
                pageType = "107";
        ...
        if(INC.config.Product_id != undefined){
        ...
        INC.methods.trackingEvents("bundleProductClickTracking", { "product_id": productId, "core_product_id": prdcore }, pageType,recstype,bundleid);
    };
    ...
    function prepareqty(productBlock,blockType,bundlePromoo){
        ...
        //     }, 50);
        // });
        var qty_label = document.createElement('span');
        qty_label.classList.add('qtyLabel')
        qty_label.innerText = "数量 "
        productDescQtyInputText.appendChild(qty_label);
        if (bundlePromoo != null && productBlock.getAttribute('data-sku') != INC.config.Product_id){
            var qty_select = document.createElement('select');
            qty_select.setAttribute("id", "inc_qty_select");
            for (var i=1; i<=10; i++){
                var qty_option = document.createElement('option');
                qty_option.classList.add('inc_qty_option')
                qty_option.innerHTML = parseInt(i);
                qty_option.value = parseInt(i);
                qty_select.appendChild(qty_option)
                
            }
            productDescQtyInputText.appendChild(qty_select);
            productDescQtyBlock.classList.add("inc_bundle_qty")
            qty_select.addEventListener('change',function(el){
                let tar = el.target;
                let value = tar.value;
                if(document.querySelector('.inc_promo_qty_value') != null){
                    document.querySelector('.inc_goes_img_block .inc_promo_qty_value').innerText = value
                }
                setTimeout(function () {
                    qtyupdateInc(value,productBlock,elAddText)
                }, 50);
            })
        }
        if(onloadpdp == falseflag){
            setTimeout(function(){
                if(document.querySelector('#inc_qty_select .inc_qty_option') != null){
                    let optLen = document.querySelectorAll('#inc_qty_select .inc_qty_option')
                    for (let g=0; g<=optLen.length; g++){
                        if (optLen[g] != undefined){
                            if (optLen[g].value == bundlePromoo){
                                optLen[g].selected = true;
                                break;
                            }
                        }
                    }
                    setTimeout(function () {
                        if (bundlePromoo != undefined && productBlock.getAttribute('data-sku') != INC.config.Product_id){
                            qtyupdateInc(bundlePromoo,productBlock,elAddText);
                            var promoQtyBlock = generate_html_tag('div','inc_promo_qty_block')
                            var promoQtyValue = generate_html_tag('span','inc_promo_qty_value')
                            promoQtyValue.innerText = bundlePromoo
                            var promoQtyText = generate_html_tag('span','inc_promo_qty_text')
                            promoQtyText.innerText = "台"
                            promoQtyBlock.appendChild(promoQtyValue)
                            promoQtyBlock.appendChild(promoQtyText)
                            if(document.querySelector('.inc_goes_img_block') != null){
                                document.querySelector('.inc_goes_img_block').appendChild(promoQtyBlock)
                            }
                        }
                    }, 50);
                }
        ...
        function qtyupdateInc(_elm,prdblk,eladtext){
            ...
            showcaseProductBlock.querySelector('.inc_product_qty_block').querySelector('input').value = parseInt(_elm);
            modalProductBlock.querySelector('.inc_product_qty_input_text input').value = parseInt(_elm);

            var id = prdblk.getAttribute('data-id');
            var qty = parseInt(_elm);
    ...
    INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock,elProductShowcaseBlock,mainprod,recstype,rank) {
        ...
        //     _parentblock.classList.add('inc_qty_drill_down')
        // }
        if(window.innerWidth < 821){
        ...
        if ((INC.config.pageType == "pdp" || INC.config.pageType == "productList") && _parentblock.classList.contains('inc_af_block') && window.innerWidth > 821){
            elImgMainImg.addEventListener("click", function (e) {
                ...
                if (e.target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.classList.contains('inc_category_blk')){
                    INC.methods.sendBundleClickTracking(productObj.mainId,null,recstype,productObj.bundleId);
                } else {
                    INC.methods.sendBundleClickTracking(productObj.mainId,'categoryRecs',recstype,productObj.bundleId);
                }
        ...
        elPromo.innerText = productObj.PromotionalMessage.split('|')[0];
            elPromo.classList.add('avail')
            var colorClass = productObj.PromotionalMessage.split('|')[1]
            elPromo.classList.add(colorClass)
        } else {
        ...
        if (productObj.bundle_promotion != null){
            var bundlePromoo = productObj.bundle_promotion
        }
        prepareqty(productBlock,blockType,bundlePromoo)
        ...
        elAddBlock.addEventListener("click", function () {
            ...
            // if (productObj.bundle_promotion != null){
            //     if (document.querySelectorAll('.inc_goes_img_block img')[1] != null){
            //         document.querySelectorAll('.inc_goes_img_block')[1].classList.add('inc_qty_drill_down')
            //     }
            //     if (document.querySelectorAll('.inc_plus_icon')[1] != null){
            //         document.querySelectorAll('.inc_plus_icon')[1].classList.add('inc_qty_drill_down')
            //     }
            // }
        });
        return productBlock;
    ...
    INC.methods.createProductModalBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock,elmodalblock,mainprod,recstype) {
        ...
        if(mainprod != 'mainprod'){
            elImgMainImg.addEventListener("click", function (e) {
                e.preventDefault()
                INC.methods.sendBundleClickTracking(productBlock.getAttribute('data-main_id'),null,recstype,productObj.bundleId);
                getTrackingPixel();
    ...
    INC.methods.addProductToCart = function (findObjd, productBlock, add_btn_click, _add_prod_im, btn_addtocart,newbtntext) {
        ...
        function addsuccess_(btn_t,addbtnt,addedprdblock,objfind,dealPrd){
            INC.config.disablebtn = true
            if (dealPrd == "deal"){
                addbtnt.innerText = "カートに追加済";
                addbtnt.classList.add('inc_added')
                btn_t.innerText = "カートに追加済";
                if(newbtntext != undefined){
                    newbtntext.innerHTML = "カートに追加済";
                }
                if (window.innerWidth > 768){
        ...
        var coreProductId = productObj.bundledProductId.toString()
        if(INC.config.Product_id != undefined){
            coreProductId = INC.config.Product_id
        }
        // var bundleids = productObj.bundleId.toString()
        if(INC.config.pageType != "cartPage"){
           let eventdata = {
                "core_product_id": coreProductId.toString(),
    ...
    INC.methods.trackingEvents = function (eventType, eventDataObj, pageType,recstype,bundleid) {
        ...
        switch (eventType) {
            ...
            case "bundleProductClickTracking":
                ...
                if (recstype == "fbtRecs"){
                    importEventDataObj["rt"] = "15";
                }
                if(bundleid != undefined){
                    let bundle_id = bundleid.toString()
                    if(bundle_id.indexOf('offer') >= 0){
                        importEventDataObj["rt"] = "11";
                    }
                }
